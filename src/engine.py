...
def pretrain_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if args.loss_mode == 'mae+triplet':
            anchor, positive, negative = [x.to(device, non_blocking=True) for x in data]
        else:
            samples, _ = data
        ...
        with amp_autocast():
            if args.loss_mode == 'mae+triplet':
                loss_mae, anchor_embed, positive_embed, negative_embed = model.forward_triplet(
                    anchor, positive, negative, mask_ratio=args.mask_ratio
                )
                triplet_loss = F.triplet_margin_loss(anchor_embed, positive_embed, negative_embed, margin=1.0)
                loss = args.alpha * loss_mae + args.beta * triplet_loss
            else:
...
def evaluate(data_loader, model, device, if_stat=False):
    ...
    print('* Pre {macro_pre:.4f} Rec {macro_rec:.4f} F1 {macro_f1:.4f}'.format(
        macro_pre=macro[0], macro_rec=macro[1], macro_f1=macro[2]))

    test_state = {k: meter.global_avg for k, meter in metric_logger.meters.items()}
    test_state.update({
        'weighted_pre': macro[0].tolist(),
        'weighted_rec': macro[1].tolist(),
        'weighted_f1': macro[2].tolist(),
        'cm': cm.tolist(),
        'acc': acc.tolist(),
        'pre_per_class': pre_per_class,
...
def evaluate_speed_test(data_loader, model, device, args):
    ...
    for i in tqdm(range(3, 11), desc="Batch size"):
        ...
        data_loader_tmp = torch.utils.data.DataLoader(
            data_loader.dataset, sampler=data_loader.sampler,
