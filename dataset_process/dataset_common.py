...
def _read_5hp_list_worker(pcap_path, if_augment, queue):
    try:
        res = read_5hp_list(pcap_path, if_augment)
        queue.put(res)
    except Exception as e:
        print(f"Error in read_5hp_list for {pcap_path}: {e}")
        queue.put(None)

def safe_read_5hp_list(pcap_path, if_augment=False, timeout=10):
    queue = multiprocessing.Queue()
    p = multiprocessing.Process(target=_read_5hp_list_worker, args=(pcap_path, if_augment, queue))
    p.start()
    p.join(timeout)
    if p.is_alive():
        print(f"Timeout in read_5hp_list for {pcap_path}, killing process.")
        p.terminate()
        p.join()
        return None
    try:
